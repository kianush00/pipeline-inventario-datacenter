...
2. **Cumplimiento de `GEMINI.md`:** Todo el código evaluado debe adherirse ciegamente a las 8 directrices del archivo maestro `GEMINI.md`. Si encuentras violaciones a tipado estático, falta de manejo de errores, baja idempotencia, redundancias o mala modularidad, debes rechazarlo. Presta especial atención a la sección de **Pruebas Unitarias**: verifica que los nuevos tests aporten valor real, que no sean redundantes y que todos los imports de `pytest` o del código fuente en los archivos de test estén declarados en el ámbito global (sin anidar dentro de las funciones).
