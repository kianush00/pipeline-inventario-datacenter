...
4. **Idempotencia y Eficiencia:** Asegúrate de que las pruebas unitarias que generes no sean redundantes con las que ya existen (revisa el archivo de tests correspondiente antes de añadir nuevas). Utiliza `MagicMock` y dependencias aisladas para simular el comportamiento de bases de datos como NetBox.

## Resultado Esperado

Debes proporcionar los bloques de código para agregar a la suite de `pytest`, debidamente comentados explicando qué escenario destructivo validan. Al finalizar, debes instruir explícitamente ejecutar la suite de pruebas para confirmar tus hallazgos.
